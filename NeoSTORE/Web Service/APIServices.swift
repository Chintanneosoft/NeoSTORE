...
enum APIServices{
    ...
    var path: String{
        ...
        var httpMethod: String {
            switch self {
            case .userLogin,.userRegister,.setRatings,.addToCart,.updateCart,.deleteCart:
                return "POST"
            default:
                return "GET"
            }
        ...
        var param: [String:Any]? {
            switch self {
            case .userRegister(param: let param), .userLogin(let param),.fetchProductsList(param: let param),.fetchProductsDetails(param: let param),.setRatings(param: let param),.addToCart(param: let param),.updateCart(param: let param),.deleteCart(param: let param):
                return param
            default:
                return nil
            }
